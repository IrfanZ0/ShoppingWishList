package com.halalark.shoppingwishlist.shoppingwishlist
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            ShoppingWishListTheme(darkTheme = isDarkModeOn, dynamicColor = isDynamicColorOn) {
                ...
                userDataVM.getAllRecipient(email)
                val recipients = userDataVM.recipientsDataState.collectAsState().value


                isDarkModeOn = mapsVM.setDarkMode.collectAsState().value

                Showscaffold(purchaserData, recipients, mapsVM, userDataVM, isDarkModeOn)
            }
        }
    }
}
...
fun GoogleMapsPreview() {
    ShoppingWishListTheme {
        ...
        val recipientData = mutableListOf(
            RecipientData(
                "R.drawable.baseline_emoji_blue_face",
                "Naveed",
                "Syed",
                "16364 Index Street",
                "Granada Hills",
            ...
            RecipientData(
                "R.drawable.baseline_emoji_orange_face",
                "Syed",
                "Ziaulla",
...
fun Showscaffold(
    ...
    recipientData: MutableList<RecipientData>,
...
fun CreateRecipientFilter(
    recData: MutableList<RecipientData>,
...
fun LoadGoogleMaps(
    ...
    recipientDataList: MutableList<RecipientData>,
    mapsVM: MapsViewModel
) {
    val context = LocalContext.current
        ...
        GoogleMap(
            ...
            if (showFilteredMarkers) {
                ...
                recipientDataList.forEach { data ->
                    val recIconId = context.resources.getIdentifier(
                        data.recipientImage.substringAfterLast("."),
                        "drawable",
                        context.packageName
                    )
                    ShowMarker(
...
fun ShowMarker(position: LatLng, title: String, isVisible: Boolean, vectorResId: Int) {
    ...
    val icon = remember(vectorResId) {
        if (vectorResId != 0 && !isPreview) {
